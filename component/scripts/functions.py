...
range_constraint_names = [i for i in cp.criterias if cp.criterias[i]['content'] == 'RANGE']
bool_constraint_names = [i for i in cp.criterias if cp.criterias[i]['content'] == 'BINARY']
...
def wlc(layer_list, constraints, priorities, aoi_ee):
    ...
    benefit_list = [i for i in layer_list if i['theme'] == 'benefits' and priorities[i['id']] != 0]
    list(map(lambda i : i.update({'eeimage':ee.Image(i['layer']).unmask() }), benefit_list))
    ...
    list(map(lambda i : i.update({'norm_weight': round( (priorities[i['id']] / sum_weights), 5) }), benefit_list))
    ...
    wlc_image = _percentile(wlc_image, aoi_ee, scale=10000, percentile=[3,97]).multiply(4).add(1)

    # rather than clipping paint wlc to region
    wlc_out = ee.Image().float()
    wlc_out = wlc_out.paint(ee.FeatureCollection(aoi_ee), 0).where(wlc_image, wlc_image).selfMask()
...
def set_constraints(constraints, constraint_list):
    ...
    for name in constraints:
        ...
        # range constraints (slope, rainfall, etc...)
            ...
            constraint_layer.update(eeimage = get_range_constraint(value, constraint_layer['layer']))
        ...
        # the rest of the bool values (land cover have already been handle I can safely use the bool list)
            ...
            constraint_layer.update(ee_image = get_bool_constraint(value, constraint_layer['layer']))

    # add the default geographic constraint 
    default_geographic = next(item for item in constraint_list if item["name"] == 'Current tree cover less than potential')
...
def get_layer(layer_name, constraint_list):
    ...
    if layer_name in  cp.landcover_default_cat:
        constraint_layer = next(i for i in constraint_list if i["name"] == 'Current land cover')
